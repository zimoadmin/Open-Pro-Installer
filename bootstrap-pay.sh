...
#
# 安装过程默认只显示：免责声明、支付流程、错误/警告，
# 想看完整进度（[STEP]/[OK] 那些行）就跑：
#   curl -fsSL https://att.12334123.xyz/bootstrap.sh | OPI_VERBOSE=1 sh
#
# 正常启动流程静默化版本
#
...
spin_start()
{
    ...
    if [ "${SPIN_MSG_SHOWN:-0}" != "1" ]; then

        SPIN_MSG_SHOWN=1

        printf "%b\n" "${CYAN}正在安装，请稍候（下载 + 解压，视网络约 10~60 秒）...${RESET}"

    fi

    # 默认不打心跳点；想看进度就 OPI_VERBOSE=1
    [ "${OPI_VERBOSE:-0}" = "1" ] ||
        return 0
