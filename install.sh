...
other_tools_menu()
{
    ...
    do
        ...
        printf "%b\n" "${BLUE}║${CYAN}  [3] E5800 屏幕中文                  ${BLUE}║${RESET}"
        printf "%b\n" "${BLUE}║${CYAN}  [0] 返回主菜单                      ${BLUE}║${RESET}"
        printf "%b\n" "${BLUE}╚══════════════════════════════════════╝${RESET}"
        ...
        case "$TOOL_CHOOSE" in
            ...
            3)
                clear
                printf "\n"
                printf "%b\n" "${GREEN}===== E5800 屏幕中文 =====${RESET}"
                printf "\n"

                if [ -f "$SCRIPT_DIR/modules/e5800_screen_zh.sh" ]; then
                    . "$SCRIPT_DIR/modules/e5800_screen_zh.sh"
                    if command -v install_e5800_screen_zh >/dev/null 2>&1; then
                        install_e5800_screen_zh
                    else
                        printf "%b\n" "${RED}E5800模块缺少 install_e5800_screen_zh()${RESET}"
                    fi
                else
                    printf "%b\n" "${RED}E5800屏幕中文模块不存在${RESET}"
                    printf "%b\n" "${YELLOW}缺少：modules/e5800_screen_zh.sh${RESET}"
                fi
                pause
            ;;
